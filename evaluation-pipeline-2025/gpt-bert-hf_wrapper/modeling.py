...
class BertModel(BertPreTrainedModel):
    def __init__(self, config, **kwargs):
        ...
        self.embedding   = Embedding(config)
        self.transformer = Encoder(config, activation_checkpointing=config.gradient_checkpointing)

    def forward(self, input_ids, attention_mask=None, **kwargs):
        static_embs, rel_embs = self.embedding(input_ids)
        mask = attention_mask.bool().unsqueeze(1) if attention_mask is not None else None
        last_hidden = self.transformer(static_embs, mask, rel_embs)
        return BaseModelOutput(last_hidden_state=last_hidden)
...
class BertForMaskedLM(BertPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = ["cls.net.5.weight"]
    ...
    def forward(self, input_ids, attention_mask=None, labels=None, **kwargs):
        static_embs, rel_embs = self.embedding(input_ids)
        seq_len = static_embs.size(1)
        attn_mask_2d = torch.zeros(
            (seq_len, seq_len),
            dtype=torch.bool,
            device=static_embs.device
        )
        static_embs = static_embs.transpose(0, 1)
        hidden = self.transformer(static_embs, attn_mask_2d, rel_embs)
        hidden = hidden.transpose(0, 1)
        label_ids = labels if labels is not None else torch.zeros_like(input_ids, dtype=torch.long)
        flat_logits = self.classifier(hidden, label_ids)
        batch_size, seq_len = input_ids.size()
        vocab_size = flat_logits.size(-1)
        logits = flat_logits.view(batch_size, seq_len, vocab_size)
    ...
    # Leave here as a error checkpoint :)
    # def forward(self, input_ids, attention_mask=None, labels=None, **kwargs):
    #     static_embs, rel_embs = self.embedding(input_ids)
    #     mask = attention_mask.bool().unsqueeze(1) if attention_mask is not None else None
    #     hidden = self.transformer(static_embs, mask, rel_embs)
    #     logits = self.classifier(hidden, labels)
