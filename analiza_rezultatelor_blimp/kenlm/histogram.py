...
if __name__ == '__main__':
    ...
    print_bins(bin_main)
    save_bins_to_json(bin_main, 'KenLM_blimp_bins.json')
    plot_histogram(
        [s for _, s in main_entries],
        bins_main, labels_main,
        title='BLiMP',
        save_path = "KenLM_blimp_histogram.pdf"
    )

    bin_supp = bucket_tasks(supp_entries, bins_main, labels_main)
    print_bins(bin_supp)
    save_bins_to_json(bin_supp, 'KenLM_blimp_supplement_bins.json')
    plot_histogram(
        [s for _, s in supp_entries],
        bins_main, labels_main,
        title='BLiMP Supplement',
        save_path = "KenLM_blimp_supplement_histogram.pdf"
    )
